...
export function parseIngredient(
  ...
  // Extract trailing parenthetical content before parsing to prevent parsing errors
  // We'll add it to extra after parsing if quantity was successfully extracted
  let trailingParentheticalContent = "";
  let textToParse = text;
  if (text && typeof text === "string") {
    const extracted = extractTrailingParentheses(text);
    trailingParentheticalContent = extracted.content;
    textToParse = extracted.cleanedText;
  }

  const tokens = tokenize(textToParse, false);
  ...
  let [ingredient, ingredientEndIndex] = getIngredient(
    tokens,
  ...
  // If quantity was successfully extracted, add the trailing parenthetical content to extra
  if (quantity > 0 && trailingParentheticalContent) {
    // Combine with existing extra content if any
    extra = extra ? `${extra} ${trailingParentheticalContent}` : trailingParentheticalContent;
  }
...
function getQuantity(tokens, units, index = 0) {
  ...
  for (; index < tokens.length; index++) {
    ...
    if (isNumber || isFraction || isSpecialFraction || isTextNumber) {
      ...
      units.ingredientQuantityAddMarker.includes(item)
    ) {
      quantityText += `${space}${item}`;
    } else if (
      quantityText.length > 0 &&
...
/**
 * Extracts parenthetical content at the end of a string, handling nested parentheses
 * @param {string} text - The text to extract parentheses from
 * @returns {{content: string, cleanedText: string}} The content within the outermost parentheses at the end, and the text with parentheses removed
 */
function extractTrailingParentheses(text) {
  if (!text || typeof text !== "string") {
  ...
  // Work backwards from the end to find matching opening parenthesis
  for (let i = trimmed.length - 1; i >= 0; i--) {
    ...
    if (char === ")") {
      ...
      if (depth === 0) {
        ...
        const content = trimmed.substring(i + 1, startIndex);
        // Remove the parentheses from the text
        const cleanedText = (trimmed.substring(0, i) + trimmed.substring(startIndex + 1)).trim();
        return { content: content.trim(), cleanedText };
      }
    }
  }

  // If we couldn't find a matching opening parenthesis, return original text
  return { content: "", cleanedText: text };
}
